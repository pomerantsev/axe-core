...
export default function createGrid(
  ...
  const treeWalker = document.createTreeWalker(
    ...
    window.NodeFilter
      .SHOW_ELEMENT /* | window.NodeFilter.SHOW_DOCUMENT_FRAGMENT*/,
    null,
    false
  );
...
function createStackingOrder(vNode, parentVNode, treeOrder) {
  const parent =
    parentVNode.actualNode.nodeType === 11 ? parentVNode.parent : parentVNode;
  const stackingOrder = parent._stackingOrder.slice();
  ...
  if (isStackingContext(vNode, parent)) {
  ...
  const stackLevel = getStackLevel(vNode, parent);
  if (stackLevel !== null) {
